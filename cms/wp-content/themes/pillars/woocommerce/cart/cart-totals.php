	...
	<?php endif; */ ?>
		...
		if (WC()->cart->get_fees()) {
			foreach (WC()->cart->get_fees() as $fee) : ?>
				<tr class="fee">
					<td><?= esc_html($fee->name) ?></td>
					<td data-title="<?= esc_attr($fee->name) ?>"><?php wc_cart_totals_fee_html($fee); ?></td>
				</tr>
			<?php
			endforeach;
		} else if (WC()->session->get('pillars_discount')) {
			$discount = theplugin_wc_cart_total_fees(true); ?>
			<tr class="fee">
				<td colspan="2"><?= esc_html($discount['label']); ?></td>
				<td><?= wc_price($discount['discount']) ?></td>
			</tr>
		<?php } ?>
