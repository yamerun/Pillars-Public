	...
	if (defined('PILLARS_DIR')) {
		switch ($key) {
			...
			case 'logo':
				...
				break;
			default:
				if (strpos($key, 'contact-icon-') !== false) {
					require get_template_directory() . '/includes/svgdata/contact.php';
				}
				if (strpos($key, 'social-icon-') !== false) {
					require get_template_directory() . '/includes/svgdata/social-icons.php';
				}
				if (strpos($key, 'advantage-') !== false || strpos($key, 'product-') !== false || strpos($key, 'arrow-') !== false) {
